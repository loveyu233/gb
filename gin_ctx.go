...
// GetGinContextTokenClaims 获取token的claims
func GetGinContextTokenClaims(c *gin.Context) (Claims, error) {
	value, exists := c.Get("token_info")
	if !exists {
		return Claims{}, errors.New("token信息不存在")
	}
	claims, exists := ConvertAnyType[Claims](value)
	if !exists {
		return Claims{}, errors.New("token信息类型错误")
	}
	return claims, nil
}
